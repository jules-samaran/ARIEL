...
content_img = image_loader("./Images/dancer.jpg")
...
def get_content_loss(cnn, normalization_mean, normalization_std,
                               style_img, content_img,
                               content_layers=content_layers_default):
    cnn = copy.deepcopy(cnn)
...
def draw_line(image,x1,y1,x2,y2):
    numpy_image=np.array(unloader(output.type(torch.FloatTensor).view(3, imsize, imsize)))
    cv.line(numpy_image,(x1,y1),(x2,y2))

def run_segment_optimizer(cnn, normalization_mean, normalization_std,
                       content_img, input_img, num_steps=300):
    """Run the style transfer."""
    print('Building the style transfer model..')
    model, content_losses = get_content_loss(cnn,
        normalization_mean, normalization_std, style_img, content_img)
    optimizer = get_input_optimizer(input_img)
